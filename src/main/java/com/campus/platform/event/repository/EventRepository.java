...
public interface EventRepository extends JpaRepository<Event, UUID> {
    ...
    // College student — own events + open events
    @Query("SELECT e FROM Event e WHERE e.college.collegeId = :collegeId " +
            "OR e.isOpenToAll = true")
    List<Event> findVisibleEventsForCollege(@Param("collegeId") UUID collegeId);

    // Guest student — only open events
    List<Event> findAllByIsOpenToAllTrue();

    @Query("SELECT e FROM Event e WHERE " +
            "(e.college.collegeId = :collegeId OR e.isOpenToAll = true) " +
            "AND e.status = :status")
    ...
    Page<Event> findAllByIsOpenToAllTrueAndStatus(EventStatus status, Pageable pageable);
}
